...
import code.ponfee.commons.jce.pkcs.CMS;
...
public class CMSTester {
    ...
    public @Test void testEnvelop() throws Exception {
        ...
        byte[] enveloped = CMS.envelop(data, cert, new ASN1ObjectIdentifier("1.2.840.113549.3.7"));
        ...
        byte[] unveloped = CMS.unenvelop(enveloped, privateKey);
    ...
    public @Test void testSign() throws Exception {
        ...
        byte[] data = Streams.file2bytes(MavenProjects.getTestJavaFile(CMSTester.class));
        System.out.println("origin len------------" + data.length);
        byte[] signed = CMS.sign(data, privateKey, certChain);
